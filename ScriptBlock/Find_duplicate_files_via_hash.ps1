...

{
    ...
    { 
    $d.Group | Select-Object -Property Algorithm,Hash,Path
    } 

$date = Get-Date -Format "yyyy/MM/dd"
$itemstomove = $result | 
Out-GridView -Title `
"Select files (CTRL for multiple) and press OK. Selected files will be moved to $filepath\Duplicates_$date" `
-PassThru 
...
-Path $filepath\Duplicates_$date -Force
Move-Item $itemstomove.Path `
-Destination $filepath\Duplicates_$date -Force
''
Write-Warning `
"Mission accomplished. Selected files moved to C:\Duplicates_$date"

Start-Process "$filepath\Duplicates_$date"
}
